...
void CSkeletonBasics::doGestures(Vector4 h, Vector4 l, Vector4 r, Vector4 w)
{
	...
	if (come == true)
	{
		if ((h.z - l.z > .2f) && (h.z-l.z < .4f) /*&& (l.y-h.y >.4f)*/)
		{
			if (!ser.go(w.x,w.z))
				OutputDebugString(TEXT("failed to send\n"));
			printf("following now\n");
			//for (int i = 0; i < 1000; i++)
			//	printf("yes\n");
			follow = true;
			come = false;
...
void CSkeletonBasics::ProcessSkeleton()
{
	...
	if (numberTracked < 0)
	{
		...
		for (int i = 0 ; i < NUI_SKELETON_COUNT; ++i) 
		{
			...
			{
			   numberTracked = i;
			   printf("numberTacked is %d\n", numberTracked);
			}
		}
	}
	else
	{
		righthand = skeletonFrame.SkeletonData[numberTracked].SkeletonPositions[NUI_SKELETON_POSITION_HAND_RIGHT];
		...
		if (follow)
		{
			printf("shit will happen for the robot\n");
			//add code here for byte code
		}
	}

	if (skeletonFrame.SkeletonData[numberTracked].eTrackingState != NUI_SKELETON_TRACKED)
		numberTracked = -1;
     /*gestures*/
